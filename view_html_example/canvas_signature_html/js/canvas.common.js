 ...
 *
 * 在引入该JS后需要进行初始化动作
 * $().esign(canvasEdit, sign_show, sign_clear, sign_ok, alertBox,canvasWidth,canvasHeight);
...
(function (root, doc, factory) {
    ...
    $.fn.esign = function(canvasEdit, sign_show, sign_clear, sign_ok, alertBox,canvasWidth,canvasHeight) {

        //初始化相关DOM，获得结果寄存命名空间
        var domMap = domInit(canvasEdit, sign_show, sign_clear, sign_ok, alertBox,canvasWidth,canvasHeight);
        //获取画板引用
        ...
        */
        ...
        function domInit(canvasEdit, sign_show, sign_clear, sign_ok, alertBox,canvasWidth,canvasHeight){
            var domMap = {};
            ...
            if(!alertBox){
                domMap.alertBox = $("#alertBox");
            } else{
                domMap.alertBox = $("#" + alertBox);
            }
            
            if(!canvasWidth){
                domMap.canvasWidth = 80;
            } else{
                domMap.canvasWidth = canvasWidth;
            }
            
            if(!canvasHeight){
                domMap.canvasHeight = 50;
            } else{
                domMap.canvasHeight = canvasHeight;
            }
            return domMap;
        }
        ...
        function convertCanvasToImage(canvas){
            var image = new Image();
            image.width = domMap.canvasWidth;
            image.height = domMap.canvasHeight;
        ...
        function alertboxClose(){
        	domMap.alertBox.removeClass("mui-active");
        }
    };
    return $.fn.esign;
}));